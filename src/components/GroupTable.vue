...
const onMappingStateChange = (event: any, index: any, index2: any) => {
    if (!event.target?.checked) {
        delete data["response"][index2]["Assignment"][index]
    } else {
        data["response"][index2]["Assignment"][index] = {
            id: index,
            sol_maxcase: false,
    ...
    <table class="Table" v-if="data.status === 'success'">
        <thead>
            ...
            <tr>
                ...
                <th class="UserHeader" style="z-index:10" v-for="(renderdata, index) in data?.response" :key="index">
        ...
        <tbody>
            <tr v-for="(Propo, index) in Prob.response" :key="index">
                <td class="UserHeader" style="z-index:10">{{ Propo.Name }}</td>
